...
## Шаг 2: Обновляем TypeScript типы

-- Добавляем поле "Дата въезда в Китай" в таблицу visas
-- Это поле позволяет указать реальную дату въезда в Китай (по паспорту)
-- По умолчанию равно arrival_datetime, но может быть изменено координатором

-- 1. Добавляем поле в таблицу visas
ALTER TABLE visas ADD COLUMN actual_entry_date DATE;

-- 2. Создаем RPC функцию для обновления даты въезда
...
DECLARE
  ...
  arrival_date date;
BEGIN
  -- Получаем роль пользователя напрямую из auth.users
  SELECT role INTO current_role
  FROM auth.users
  WHERE id = auth.uid();

  -- Проверяем права доступа
  IF current_role NOT IN ('coordinator', 'director', 'super_admin') THEN
    RETURN json_build_object(
      'success', false, 
      'error', 'Недостаточно прав для обновления даты въезда'
    );
  END IF;

  -- Получаем дату прибытия для валидации
  SELECT tc.arrival_datetime::date INTO arrival_date
  FROM public.tickets_to_china tc
  WHERE tc.deal_id = p_deal_id;

  -- Валидация: дата въезда не может быть позже даты прибытия
  IF p_entry_date > arrival_date THEN
    RETURN json_build_object(
      'success', false, 
      'error', 'Дата въезда в Китай не может быть позже даты прибытия в клинику'
    );
  END IF;

  -- Обновляем дату въезда
  UPDATE public.visas 
  SET actual_entry_date = p_entry_date,
      updated_at = NOW()
  ...
  ELSE
    RETURN json_build_object(
      'success', false, 
      'error', 'Запись визы не найдена'
    );
  END IF;
END;
$function$;

-- Предоставляем права на выполнение RPC функции
GRANT EXECUTE ON FUNCTION public.update_china_entry_date(bigint, date) TO authenticated;

-- 3. Обновляем master view функции с новыми формулами
-- Сначала удаляем зависимые функции
DROP FUNCTION IF EXISTS public.get_filtered_master_view();

-- Обновляем основную функцию get_super_admin_master_view
...
RETURNS TABLE(
    ...
    china_entry_date date,
    visa_expiry_date date, 
    last_day_in_china date,
...
BEGIN
    ...
    SELECT 
        ...
        -- COMPUTED FIELDS - ОБНОВЛЕННЫЕ ФОРМУЛЫ С УЧЕТОМ actual_entry_date
        COALESCE(v.actual_entry_date, tc.arrival_datetime::date) as china_entry_date,
        (COALESCE(v.actual_entry_date, tc.arrival_datetime::date) + v.visa_days - 1) as visa_expiry_date,
        (COALESCE(v.actual_entry_date, tc.arrival_datetime::date) + v.visa_days - 1) as last_day_in_china,
        ((COALESCE(v.actual_entry_date, tc.arrival_datetime::date) + v.visa_days - 1) - CURRENT_DATE) as days_until_visa_expires,
        ...
        -- Visa status based on expiry (обновленная формула)
        CASE 
            WHEN (COALESCE(v.actual_entry_date, tc.arrival_datetime::date) + v.visa_days - 1) < CURRENT_DATE THEN 'Expired'
            WHEN ((COALESCE(v.actual_entry_date, tc.arrival_datetime::date) + v.visa_days - 1) - CURRENT_DATE) <= interval '3 days' THEN 'Expiring Soon'
            ELSE 'Active'
        END as visa_status,
...
-- Восстанавливаем зависимую функцию get_filtered_master_view
