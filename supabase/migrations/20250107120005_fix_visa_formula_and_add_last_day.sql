...
-- Исправляем формулу расчета даты истечения визы и добавляем новое поле
-- Старая формула: (tc.arrival_datetime::date + INTERVAL '1 day' * v.visa_days)
-- Новая формула: (tc.arrival_datetime::date + v.visa_days - 1)
...
BEGIN
    ...
    SELECT 
        ...
        -- COMPUTED FIELDS - ИСПРАВЛЕННАЯ ФОРМУЛА
        (tc.arrival_datetime::date + v.visa_days - 1) as visa_expiry_date,
        (tc.arrival_datetime::date + v.visa_days - 1) as last_day_in_china,
        ((tc.arrival_datetime::date + v.visa_days - 1) - CURRENT_DATE) as days_until_visa_expires,
        ...
        CASE 
            WHEN (tc.arrival_datetime::date + v.visa_days - 1) < CURRENT_DATE THEN 'Expired'
            WHEN ((tc.arrival_datetime::date + v.visa_days - 1) - CURRENT_DATE) <= interval '3 days' THEN 'Expiring Soon'
            ELSE 'Active'
        END as visa_status,
